...
const spock = new Hamburguer("Spock", 34.99, [
  "Pão",
  "Hamburguer",
  "Rúcula",
  "Tomate Seco",
...
const milkShake = new Bebida("Milkshake de Chocolate", 19.99, "500ml");
...
function listarHamburgueres() {
  ...
  cardapio.hamburgueres.forEach((hamburguer) => {
    const htmlIem = criarHtmlDoProduto(hamburguer);
    listaHamburgueres.appendChild(htmlIem);
  });
}
...
function listarBebidas() {
  ...
  cardapio.bebidas.forEach((bebida) => {
    const htmlItem = criarHtmlDoProduto(bebida);
